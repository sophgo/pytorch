      ...
      std::enable_if_t<
          std::is_convertible<
              ...
              c10::function_ref<
                  void(char**, const int64_t* strides, int64_t size)>>::value,
          int> = 0>
  void for_each_acc(loop1d_t loop, sync_acc_t sync, int64_t grain_size = at::internal::GRAIN_SIZE) {
    for_each_acc(loop_2d_from_1d(loop), sync, grain_size);
  }

  void for_each_acc(loop2d_t loop, sync_acc_t sync, int64_t grain_size = at::internal::GRAIN_SIZE);
